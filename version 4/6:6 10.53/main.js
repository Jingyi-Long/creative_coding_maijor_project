...
function updateInstrumentVisual(inst, s) {
  ...
  const flickerBrightness = 0.6;
  const flickerSaturate = 0.8;
  const flickerGlow = 14;
  view.wrap.style.filter =
    `brightness(${(1 + f * flickerBrightness).toFixed(3)}) ` +
    `saturate(${(1 + f * flickerSaturate).toFixed(3)}) ` +
    `drop-shadow(0 0 ${(f * flickerGlow).toFixed(1)}px rgba(255, 240, 200, ${(f * 0.5).toFixed(3)}))`;
