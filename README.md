...
When the cursor moves across the canvas, nearby shapes react with distinct behaviours: **circles** breathe and emit glowing ripple rings, **lines** (converted to quadratic Béziers) bend perpendicular to themselves toward the cursor, **triangles** spin faster and warm toward orange, and **rectangles** tilt toward or away from the pointer. A custom spring-damper function ensures every shape settles smoothly back to its original layout once the cursor leaves. The mapping also follows Kandinsky's colour–sound synesthesia: yellow regions react energetically (his "trumpet"), blue regions slowly and softly (his "organ"), and red regions in between.
...
- **Robert Hodgin, *Ancient Courses of Fictional Rivers*, 2022**, Art Blocks — https://www.artblocks.io/collection/ancient-courses-of-fictional-rivers-by-robert-hodgin — used as inspiration for time-based accumulation, generative visual movement, and layered particle aesthetics.
...
- **Kandinsky, W.** (1914). *Concerning the spiritual in art* (M. T. H. Sadler, Trans.). Constable. (Original work published 1911)
...
**Yuming Cong (Time-based):** ChatGPT was used to generate the initial sketch image and to assist with researching visual references and relevant literature. Claude was used to assist with parts of the programming implementation in `mechanic-time.js`, including smoothstep easing logic, bloom fade timing, optimisation of the decoration canvas layer, and code review. How it works: a `performance.now()` clock drives a `requestAnimationFrame` tick loop that sequences a 98 s timeline — shapes are revealed as outlines, colour fills with saturation rise, shapes oscillate in scale while bloom gradients expand on a Canvas 2D layer, then fill and outlines fade out in two stages — with `window.__globalFade` published each frame for `main.js` to synchronise. All design decisions, timeline structure, conceptual framing, and final implementation choices were made by the author.

**Zichen Feng (Perlin & randomness):** used ChatGPT to support idea development, code troubleshooting, and wording refinement for the Perlin noise mechanic. ChatGPT was used to explore how p5.js noise(), map(), frameCount, and noiseSeed() could be applied to create smooth instrument floating, layered circular particle movement, and a subtle breathing glow effect. ChatGPT also helped debug parts of the JavaScript structure, including separating the Perlin noise logic into perlin-noise.js and improving the interaction between the Perlin noise mechanic, SVG elements, particle canvas, and audio mechanic.
