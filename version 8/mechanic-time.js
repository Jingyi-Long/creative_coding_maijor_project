// mechanic-time.js — version 8 时间轴 v11
//
// 完整时间轴（点击后计时，98s自动循环）：
//  0  – 12s  : 所有图形以轮廓/线稿出现，无填色
//  12 – 47s  : 颜色填充 + 饱和度逐渐升高，47s时全部到100%
//  47 – 71s  : 图形来回放大缩小震荡 + 晕染轻微缩放震荡（晕染尺寸更小）
//  72 – 80s  : 图形缩回
//  80 – 97s  : 所有图形 + 晕染同步消失
//  97s       : 画面空白
//  98s       : 整体循环重置，从头开始
...
(function () {
  ...
  const BLOOM_FADE_START  = 80;   // 晕染80-88s随填色一起消失
  const BLOOM_FADE_END    = 88;
  const SHAPE_FADE_START  = 80;   // 保留（main.js兼容用）
  const SHAPE_FADE_DUR    = 17;
  const LOOP_AT           = 98;   // 98s循环重置
  ...
  const K_COLORS = [
    '#c1392a','#2f6cb0','#e7d24a','#000000',
    '#a878b8','#f0c7b8','#bb9450','#0077B6','#1D3557'
  ];
  const BG_COLOR = '#f1e9d8';
  ...
  function tryInit() {
    ...
    candidates.forEach(({ el, isUsed }, i) => {
      ...
      const peakSat      = 1.5 + sr(i * 37 + 7) * 0.7;
  ...
  function initNewShapes() {
    ...
    // 非黑色的康定斯基颜色池（三角形填充用，循环使用保证不重复相邻）
    const K_COLORS_NO_BLACK = K_COLORS.filter(c => c !== '#000000');
    let triColorIdx = 0;  // 三角形颜色轮转索引

    for (let i = 0; i < COUNT; i++) {
      const seed = i * 61 + 100;
      const type = Math.floor(sr(seed) * 4); // 0=曲线 1=直线 2=圆 3=三角
      ...
      // 三角形填充色：顺序轮转，保证每个三角形颜色不重复
      const triFillColor = K_COLORS_NO_BLACK[triColorIdx % K_COLORS_NO_BLACK.length];

      // 新图形缩放震荡参数（小幅度）
      const newScalePeak = 1.06 + sr(seed+17) * 0.08;  // 1.06-1.14x 小幅
    ...
    const pool = shapeItems.filter(x => {
      ...
      return f && f !== 'none' && !f.startsWith('url(') && f !== '#f1e9d8';
    });

    // 随机选4个，剔除最靠左上角的那个（x+y最小）
    ...
    for (const item of picked) {
      bloomItems.push({
        el:           item.el,
        color:        item.el.getAttribute('fill') || '#c1392a',
  ...
  function drawBloom(t) {
    ...
    for (const b of bloomItems) {
      ...
      const bloomScale = 1 + 0.2 * bEnv * osc;  // 极轻微缩放
      // shapeR * 2.5 控制晕染基础尺寸（原为6，缩小为2.5）
  ...
  function tick() {
    ...
    // 两阶段消失：80-88s填色消失，89-97s线条（全部）消失
    const fillFade = 1 - ss(t, 80, 88);   // 填色淡出
    const lineFade = 1 - ss(t, 89, 97);   // 线条/整体淡出
