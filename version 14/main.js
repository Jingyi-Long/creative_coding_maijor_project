...
function drawFireworkParticlesFromUsedElements() {
  ...
  for (const p0 of fireworkParticles) {
    ...
    let x = origin.x + Math.cos(angle) * p0.distance * move * origin.scale + driftX;
    let y = origin.y + Math.sin(angle) * p0.distance * move * origin.scale + driftY;

    // ★ Mouse ripple: nearby particles get pushed away from the cursor
    // (force field provided by input-mechanic.js via window.mouseInfluence)
    if (window.mouseInfluence) {
      const inf = window.mouseInfluence(x, y);
      if (inf.strength > 0) {
        const push = inf.strength * 40;
        x -= inf.dx * push;
        y -= inf.dy * push;
      }
    }
...
function updateInstrumentVisual(inst, s) {
  ...
  // ★ Mouse ripple: instrument gently rotates toward the cursor when nearby
  // (force field provided by input-mechanic.js via window.mouseInfluence)
  let mouseRot = 0;
  if (window.mouseInfluence) {
    const screenX = window.innerWidth  * (0.5 + pos.x / 100);
    const screenY = window.innerHeight * (0.5 + pos.y / 100);
    const inf = window.mouseInfluence(screenX, screenY);
    mouseRot = inf.strength * 15;
  }

  view.wrap.style.transform = `translate3d(calc(-50% + ${tx}vw), calc(-50% + ${ty}vh), 0) scale(${scale}) rotate(${(floatRot + mouseRot).toFixed(2)}deg)`;
